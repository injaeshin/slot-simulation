...
public class LineGame(ReelStrip reelStrip, PayTable payTable, ILogger<LineGame> log)
{
    ...
    {
        ProcessReelRow(buffer, col, 0, startIndex, reelLength, strip);
        ProcessReelRow(buffer, col, 1, startIndex + 1, reelLength, strip);
        ProcessReelRow(buffer, col, 2, startIndex + 2, reelLength, strip);
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static void ProcessReelRow(ThreadBuffer buffer, int col, int row, int pos, int reelLength, SymbolType[] strip)
    {
        pos = pos >= reelLength ? pos - reelLength : pos;
        var symbol = strip[pos];
        buffer.Symbols[col * 3 + row] = symbol;

        if (symbol == SymbolType.SS)
        {
            buffer.ScatterCount++;
        }
    }
